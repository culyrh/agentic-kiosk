...
Silero VAD는 16kHz 기준 정확히 512 샘플(32ms) 단위의 입력만 허용하므로, 클라이언트가 50ms 단위로 전송하는 PCM 청크를 내부 버퍼에 누적 후 512 샘플씩 슬라이딩 처리한다. 단순 무음 감지 외에 다음의 세부 정책이 반영되어 있다.

| 정책 | 값 | 근거 |
|------|-----|------|
| 캘리브레이션 워밍업 | 0.5s 버림 | 마이크 AGC 안정화 전 구간 제거 |
| 소음 측정 | 이후 1.5s RMS p75 | 순간 튀는 값을 제외한 전형적 배경 소음 측정 |
| 적응형 에너지 게이트 | 소음 p75 × 1.5 | 세션마다 환경에 맞게 자동 조정 |
...
캘리브레이션 완료 전에는 기본 에너지 게이트(0.02)를 사용하며, Silero가 start를 감지하더라도 에너지 게이트를 통과하지 못하면 발화로 인정하지 않는다.

---

### 2. 하이브리드 검색 분기

`search_menu`는 `query` 파라미터 유무에 따라 검색 경로를 자동 분기한다.
...
structured output 모드에서 스트리밍 토큰이 오지 않는 경우를 대비한 fallback 경로도 별도 유지한다.

---

### 4. pipeline_lock 직렬화

WebSocket 연결마다 `asyncio.Lock`을 생성해 동일 세션의 파이프라인 실행을 직렬화한다. 이를 적용하지 않으면 연속 발화 시 두 파이프라인이 동시에 `conversation_history`를 읽고 쓰면서 아래 문제가 발생한다.

```
파이프라인 A: [HumanMsg] → tool_call 삽입 → (LLM 대기 중)
파이프라인 B: [HumanMsg] → history 읽음 → A의 tool_call이 포함된 채로 LLM 호출
→ ToolMessage 없이 tool_call만 있는 상태 → OpenAI API 400 에러
```

Lock으로 직렬화하면 A가 완전히 끝난 후 B가 시작되어 항상 tool_call · tool_result 쌍이 보장된다.
