...
기존 키오스크는 계층형 UI를 직접 탐색해야 해서 디지털 취약계층에게 진입장벽이 높습니다. 이 프로젝트는 사용자의 자연스러운 음성 발화를 LangChain ReAct 에이전트가 분석·판단해 메뉴 검색, 장바구니 관리, 결제까지 전 과정을 자율적으로 처리하는 무인 단말기용 음성 주문 시스템입니다.

> 본 프로젝트는 2026-1 산학실전 캡스톤디자인 프로젝트로 개발되었습니다. ([Front-End](https://github.com/seb0070/sadollar-kiosk-fe))
...
사용자 발화를 받은 에이전트가 의도를 판단해 직접 호출하는 함수 목록이다. 하나의 발화에서 여러 tool이 순서대로 실행될 수 있으며, 각 tool의 실행 결과가 다음 판단의 입력으로 이어진다.
